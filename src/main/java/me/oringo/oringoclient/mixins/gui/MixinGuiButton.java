...
public abstract class MixinGuiButton extends GuiMixin
{
    ...
    public void drawButton(final Minecraft mc, final int mouseX, final int mouseY, final CallbackInfo callbackInfo) {
        if (this.visible && OringoClient.interfaces.customButtons.isEnabled()) {
            ...
            try {
                Fonts.robotoMediumBold.drawSmoothCenteredString(this.displayString, this.xPosition + this.width / 2.0f, this.yPosition + (this.height - Fonts.robotoMediumBold.getHeight()) / 2.0f, this.hovered ? Color.white.getRGB() : new Color(200, 200, 200).getRGB());
            }
            catch (Exception e)
            {
                e.printStackTrace();
                System.out.println("Fonts is null");
            }
            GlStateManager.color(1.0f, 1.0f, 1.0f, 1.0f);
            callbackInfo.cancel();
        }
        ...
        if (OringoClient.interfaces.buttonLine.is("Wave")) {
            ...
            for (int i = 1; i < 11; ++i) {
                ...
                if (OringoClient.interfaces.lineLocation.is("Top")) {
                    this.addVertexes(prevPos - expand, this.yPosition - expand, pos + expand, this.yPosition + 1.5f + expand, RenderUtils.applyOpacity(OringoClient.clickGui.getColor(i), opacity).getRGB(), RenderUtils.applyOpacity(OringoClient.clickGui.getColor(i + 1), opacity).getRGB());
                }
                else {
                    this.addVertexes(prevPos - expand, this.yPosition - expand + this.height - 1.5f, pos + expand, this.yPosition + this.height + expand, RenderUtils.applyOpacity(OringoClient.clickGui.getColor(i), opacity).getRGB(), RenderUtils.applyOpacity(OringoClient.clickGui.getColor(i + 1), opacity).getRGB());
                }
                prevPos = pos;
            }
            this.end2ColorDraw();
            ...
            if (OringoClient.interfaces.lineLocation.is("Top")) {
                RenderUtils.drawRect(this.xPosition - expand, this.yPosition - expand, this.xPosition + this.width + expand, this.yPosition + 1.5f + expand, RenderUtils.applyOpacity(OringoClient.clickGui.getColor(), opacity).getRGB());
            }
            else {
                RenderUtils.drawRect(this.xPosition - expand, this.yPosition - expand + this.height - 1.5f, this.xPosition + this.width + expand, this.yPosition + this.height + expand, RenderUtils.applyOpacity(OringoClient.clickGui.getColor(), opacity).getRGB());
            }
        }
    }
