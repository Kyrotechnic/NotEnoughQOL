...
public abstract class GuiNewChatMixin extends GuiMixin
{
    ...
    private List<IChatComponent> onFunc(final IChatComponent k, final int s1, final FontRenderer chatcomponenttext, final boolean l, final boolean chatcomponenttext2) {
        return (OringoClient.interfaces.customChatFont.isEnabled() && OringoClient.interfaces.isToggled() && OringoClient.interfaces.customChat.isEnabled()) ? this.wrapToLen(k, s1, chatcomponenttext) : GuiUtilRenderComponents.splitText(k, s1, chatcomponenttext, l, chatcomponenttext2);
    }
