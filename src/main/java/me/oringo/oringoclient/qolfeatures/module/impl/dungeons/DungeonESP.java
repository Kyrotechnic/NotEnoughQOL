...
public class DungeonESP extends Module
{
    ...
    public void onUpdate(final MotionUpdateEvent.Pre event) {
        if (DungeonESP.mc.thePlayer.ticksExisted % 20 == 0 && SkyblockUtils.inDungeon) {
            ...
            for (final Entity entity2 : DungeonESP.mc.theWorld.loadedEntityList.stream().filter(entity -> entity instanceof EntityLivingBase).collect(Collectors.toList())) {
    ...
    public void onRenderWorld(final RenderWorldLastEvent event) {
        ...
        if (this.isToggled() && SkyblockUtils.inDungeon)
        {
