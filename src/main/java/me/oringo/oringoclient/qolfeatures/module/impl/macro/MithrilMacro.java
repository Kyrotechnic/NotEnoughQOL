...
public class MithrilMacro extends Module
{
        ...
        if (this.drillRefuel.isEnabled() && ChatFormatting.stripFormatting(message).startsWith("Your") && ChatFormatting.stripFormatting(message).endsWith("Refuel it by talking to a Drill Mechanic!") && this.drillnpc != null) {
            ...
            new Thread(() -> {
                ...
                try {
                    ...
                    Thread.sleep(500L);
                    this.mc.playerController.interactWithEntitySendPacket((EntityPlayer)this.mc.thePlayer, (Entity) (Object) this.drillnpc);
                    Thread.sleep(2500L);
        ...
        if (this.isToggled() && !(this.mc.currentScreen instanceof GuiContainer) && !(this.mc.currentScreen instanceof GuiEditSign) && this.pause < 1) {
            ...
            if (this.mc.theWorld != null) {
                if (this.drillnpc == null && this.drillRefuel.isEnabled()) {
                    for (final Entity entityArmorStand : this.mc.theWorld.getLoadedEntityList().stream().filter(entity -> entity instanceof EntityArmorStand).collect(Collectors.toList())) {
                ...
                if (this.mc.theWorld.playerEntities.stream().anyMatch(playerEntity -> !playerEntity.equals((Object)this.mc.thePlayer) && playerEntity.getDistanceToEntity((Entity) (Object) this.mc.thePlayer) < 10.0f && EntityUtils.isTeam((EntityLivingBase)playerEntity) && (!playerEntity.isInvisible() || playerEntity.posY - this.mc.thePlayer.posY <= 5.0))) {
                    ++this.ticksSeen;
                }
                else {
                    this.ticksSeen = 0;
    ...
    private boolean findTarget() {
        ...
        final BlockPos pos2 = OringoClient.mc.thePlayer.getPosition();
        ...
        if (any.isPresent()) {
            ...
            final BlockPos pos3 = OringoClient.mc.thePlayer.getPosition();
