        ...
        if (this.isToggled()) {
            if (event.isPre()) {
                ...
                if (AutoHeal.generalDelay.hasTimePassed(500L) && AutoHeal.powerOrb.isEnabled() && AutoHeal.mc.thePlayer.getHealth() / AutoHeal.mc.thePlayer.getMaxHealth() <= AutoHeal.orbHp.getValue() / 100.0) {
                    final List<EntityArmorStand> stands = (List<EntityArmorStand>)AutoHeal.mc.theWorld.getEntities(EntityArmorStand.class, e -> getPowerLevel(e.getName()) < 4 && e.getDistanceToEntity((Entity)AutoHeal.mc.thePlayer) < 20.0f);
