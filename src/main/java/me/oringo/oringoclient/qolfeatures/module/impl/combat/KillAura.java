...
import me.oringo.oringoclient.qolfeatures.module.impl.dungeons.Aimbot;
...
public class KillAura extends Module
{
    ...
    private EntityLivingBase getTarget() {
        ...
        final List<Entity> validTargets = (List<Entity>)KillAura.mc.theWorld.getLoadedEntityList().stream().filter(entity -> entity instanceof EntityLivingBase).filter(entity -> this.isValid((EntityLivingBase) entity)).sorted(Comparator.comparingDouble(e -> e.getDistanceToEntity((Entity)KillAura.mc.thePlayer))).collect(Collectors.toList());
