...
public class ArmorStandsCommand extends Command
{
    ...
    public void execute(final String[] args) throws Exception {
        ...
        ArmorStandsCommand.mc.theWorld.loadedEntityList.stream().filter(entity -> entity instanceof EntityArmorStand && entity.getDisplayName().getFormattedText().length() > 5 && entity.hasCustomName()).sorted(Comparator.comparingDouble(ArmorStandsCommand.mc.thePlayer::getDistanceToEntity).reversed()).forEach(ArmorStandsCommand::sendEntityInteract);
    }
