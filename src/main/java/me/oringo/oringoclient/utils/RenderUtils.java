...
public class RenderUtils
{
    ...
    public static void drawImage(final ResourceLocation image, final float x, final float y, final float width, final float height, final float alpha) {
        ...
        OringoClient.mc.getTextureManager().bindTexture(image);
    ...
    public static void blockBox(final TileEntity block, final Color color) {
        ...
        GL11.glDepthMask(false);
        setColor(color);
        RenderGlobal.drawSelectionBoundingBox(block.getRenderBoundingBox().offset(-OringoClient.mc.getRenderManager().viewerPosX, -OringoClient.mc.getRenderManager().viewerPosY, -OringoClient.mc.getRenderManager().viewerPosZ));
        GL11.glEnable(3553);
        GL11.glEnable(2929);
        GL11.glDepthMask(true);
        GL11.glDisable(3042);
    ...
    public static void renderStarredNametag(final Entity entityIn, final String str, final double x, final double y, final double z, final int maxDistance) {
        final double d0 = entityIn.getDistanceSqToEntity(OringoClient.mc.getRenderManager().livingPlayer);
